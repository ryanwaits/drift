...
const TMP = path.resolve(__dirname, '.tmp-docs');
...
describe('drift scan --map', () => {
  test('auto-loads drift.docs.json', () => {
    const { envelope, exitCode } = json(['scan', 'index.ts']);
    expect(envelope.data.docsCoverage).toBeDefined();
    expect(envelope.data.docsCoverage.pages[0].ghosts[0].key).toBe('ghostOption');
    expect(exitCode).toBe(1);
  });

  test('reports gaps/ghosts/inversions and fails on ghost', () => {
    const { envelope, exitCode } = json(['scan', 'index.ts', '--map', 'drift.docs.json']);
  ...
  test('--annotations emits ::error for ghosts with file locations', () => {
    const result = run(['scan', 'index.ts', '--map', 'drift.docs.json', '--annotations', '--json']);
  ...
  test('invalid map exits 2 with precise error', () => {
    ...
    const result = run(['scan', 'index.ts', '--map', 'bad-map.json', '--json']);
    expect(result.exitCode).toBe(2);
    expect(result.stdout.toString()).toContain('pages[0]');
  });
});

describe('drift scan --map standalone (no package entry)', () => {
  ...
  test('runs key coverage without an entry when every page has a spec', () => {
    const result = run(['scan', '--map', 'drift.docs.json', '--json'], DIR);
    const envelope = JSON.parse(result.stdout.toString());
    expect(envelope.ok).toBe(true);
    // No package under scan: coverage/lint absent, docsCoverage present
  ...
  test('--annotations emits workspace-relative file paths', () => {
    const result = Bun.spawnSync(
      ['bun', 'run', CLI, 'scan', '--map', 'drift.docs.json', '--annotations', '--json'],
...
describe('drift docs', () => {
  test('stub proposes page→type mapping by key overlap', () => {
    const { envelope } = json(['docs', 'init', '--docs', 'docs']);
    expect(envelope.ok).toBe(true);
  ...
  test('baseline tightens but never raises', () => {
    ...
    const { envelope } = json(['docs', 'baseline', 'ratchet-map.json']);
    expect(envelope.ok).toBe(true);
    ...
    const again = json(['docs', 'baseline', 'ratchet-map.json']);
