...
export function registerExamplesCommand(program: Command): void {
  program
    ...
    .action(async (entry: string | undefined, options: { typecheck?: boolean; run?: boolean; all?: boolean; private?: boolean; min?: string }) => {
      ...
      try {
        ...
        if (options.all) {
          ...
          for (const pkg of packages) {
            ...
            const result = await validateExamples(exps, { validations, packagePath, exportNames });
          ...
          const data = { packages: rows, aggregate: { score: aggScore, withExamples: totalWith, total: totalAll }, ...(skipped.length > 0 ? { skipped } : {}) };
        ...
        const next: OutputNext | undefined = totalIssues > 0
          ? { suggested: 'drift-fix-examples skill', reason: `${totalIssues} examples need attention` }
          : undefined;
        ...
        if (minThreshold !== undefined && result.presence) {
          const score = result.presence.total > 0
