      ...
      try {
        ...
        if (batchPackages && batchPackages.length > 0 && options.base) {
          ...
          for (const pkg of packages) {
            ...
            rows.push({ name: pkg.name, breaking: diff.breaking.length, added: diff.nonBreaking.length, changed: diff.docsOnly.length });
            totalBreaking += diff.breaking.length;
            totalAdded += diff.nonBreaking.length;
            totalChanged += diff.docsOnly.length;
          }

          const data = { packages: rows, aggregate: { breaking: totalBreaking, added: totalAdded, changed: totalChanged }, ...(skipped.length > 0 ? { skipped } : {}) };
