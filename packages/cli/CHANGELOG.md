...
- 8a6309c: `spec-not-in-claims`: a section walks a type's members when three or more distinct public members are named under the type's heading by code-span labels such as `` `history.undo()` ``, `x.member(` calls on any receiver, or `Type.member` text; that both joins the type and documents its surface (lively's `HistoryManager` / `Room` sections got their gaps back). Option keys do not count. A receiver the fence declares (`const history = doc.getHistory()`) is no longer mistaken for the browser global of that name.
- Updated dependencies [8a6309c]
  - @driftdev/sdk@1.19.2
...
- 31c0f45: - `spec-not-in-claims`: a section that walks a type's members (three or more distinct public members named by code-span labels, calls on a bound receiver or `Type.member` text) documents its surface and gets its gaps again (regression in 1.19.0 on reference-style guides such as lively's storage page). One example with an option key stays a guide section.
  - A migration "before" fence (`Before` / `Previous` / `Old` heading, lower version label, `// before` comment) carries no claims at all, inventory included: the old API is shown on purpose, so no question is asked of it.
- Updated dependencies [31c0f45]
  - @driftdev/sdk@1.19.1
