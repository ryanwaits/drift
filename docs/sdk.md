...
- A fence that imports a secondary's `importSpecifier` and not the primary's (`import * as z from "zod/mini"`) is checked against that spec for every rule, with its own bindings and namespace aliases; its inventory mentions are that entry's exports. Claims keep their shape: a `specRef` does not say which spec resolved it (`signature` and `slices` come from that spec).
