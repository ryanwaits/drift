...
Fence call-site rules (PageDocument only, not scan): `prose-unknown-key` (JSX prop / object-literal key not on the closed object type of the parameter at that position; no claim for `T` / `Partial<T>` / `Record` / index signatures), `prose-arity-mismatch` (more positional args than any overload), `prose-missing-required` (required in every overload; JSX `children` counts). JSX props are the component's top-level props, not nested property types. Every JSX element in the fence that resolves to an export is checked. Type arguments are not arguments (`foo<A, B>()` is zero args). Unknown receiver = no claim.
